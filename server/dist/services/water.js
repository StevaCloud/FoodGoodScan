...
exports.getWaterInfo = getWaterInfo;
exports.isWaterProduct = isWaterProduct;
exports.getPhRating = getPhRating;
const WATER_DB = {
    '3068320055008': {
        brand: 'Evian',
        ph: 7.2,
...
function getWaterInfo(barcode) {
    return WATER_DB[barcode] || null;
}
function isWaterProduct(productName, categories) {
...
function getPhRating(ph) {
    if (ph < 5.5)
        return { rating: 'Très acide', color: '#ef4444', emoji: '🔴' };
    if (ph < 6.5)
        return { rating: 'Acide', color: '#f97316', emoji: '🟠' };
    if (ph < 7.3)
        return { rating: 'Neutre (idéal)', color: '#22c55e', emoji: '🟢' };
    if (ph < 8.0)
        return { rating: 'Légèrement alcaline', color: '#84cc16', emoji: '🟢' };
    if (ph < 9.0)
        return { rating: 'Alcaline', color: '#eab308', emoji: '🟡' };
